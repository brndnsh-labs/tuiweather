...
describe("reduced_motion config", () => {
  test("defaults to false when absent", () => {
    const cfg = tuiConfigSchema.parse({ schema_version: 3 });
    expect(cfg.reduced_motion).toBe(false);
  });

  test("parses true and false", () => {
    expect(tuiConfigSchema.parse({ schema_version: 3, reduced_motion: true }).reduced_motion).toBe(
      true,
    );
    expect(tuiConfigSchema.parse({ schema_version: 3, reduced_motion: false }).reduced_motion).toBe(
      false,
    );
  });
  ...
  test("serializes and round-trips through saveConfig/loadConfig", async () => {
    ...
    try {
      ...
      const withTrue = tuiConfigSchema.parse({ schema_version: 3, reduced_motion: true });
      ...
      const withFalse = tuiConfigSchema.parse({ schema_version: 3, reduced_motion: false });
      ...
      const def = tuiConfigSchema.parse({ schema_version: 3 });
