...
describe("reduced_motion config", () => {
  ...
  test("parses true and false", () => {
    expect(tuiConfigSchema.parse({ schema_version: SCHEMA_VERSION, reduced_motion: true }).reduced_motion).toBe(
      true,
    );
    expect(tuiConfigSchema.parse({ schema_version: SCHEMA_VERSION, reduced_motion: false }).reduced_motion).toBe(
      false,
    );
  });
  ...
  test("serializes and round-trips through saveConfig/loadConfig", async () => {
    ...
    try {
      ...
      const withTrue = tuiConfigSchema.parse({ schema_version: SCHEMA_VERSION, reduced_motion: true });
      ...
      const withFalse = tuiConfigSchema.parse({ schema_version: SCHEMA_VERSION, reduced_motion: false });
