...
- **npm artifact runs on Node ≥ 26.4, not Bun.** The bundle is `--target node` (only `@opentui/core-*` native packages are external — the core JS is bundled, so CLI paths work with zero `node_modules`; native loads lazily at renderer init). Node needs `--experimental-ffi` for that native load; the bin shebang is `#!/usr/bin/env -S node --experimental-ffi --disable-warning=ExperimentalWarning` (`env -S` is portable across macOS BSD env and coreutils, and npm's cmd-shim parses multi-arg shebangs correctly on Windows — verified via `windows-smoke` CI). `process.argv` works identically under Bun, so never reintroduce `Bun.argv`; `test/package.test.ts` executes the packed artifact under plain node and is the regression guard.
